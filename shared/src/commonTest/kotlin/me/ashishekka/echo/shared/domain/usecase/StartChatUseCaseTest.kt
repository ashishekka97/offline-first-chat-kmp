...
class StartChatUseCaseTest {
    ...
    fun setup() {
        ...
        startChatUseCase = StartChatUseCase(chatRepository, agentService, mediaService, localAssetManager, stringProvider)
    }
    ...
    @Test
    fun testAutoGeneratedTitleForImage() = runTest {
        val result = startChatUseCase(
            chatId = ChatId("c1"),
            participantIds = listOf(ParticipantId("p1"), ParticipantId("p2")),
            messageId = MessageId("m1"),
            ...
            localMediaPath = "some/path.jpg"
        )

        assertEquals(Result.Success(Unit), result)
        assertEquals("shared_image_chat", chatRepository.lastTitle)
    }
